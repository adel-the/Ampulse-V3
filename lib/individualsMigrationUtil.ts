...
export async function migrateIndividualsToDatabase(
  ...
  try {
    ...
    const individuals: Individual[] = parsedData.individuals || [];
    ...
    await useIndividusHook.fetchIndividus()
    ...
    const individuInserts = individuals.map(ind => 
    ...
    return {
      success: true,
      migratedCount: individuals.length,
      existingCount: 0
    }
