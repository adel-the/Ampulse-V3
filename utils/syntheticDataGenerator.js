// Synthetic Data Generator for PMS System Testing (JavaScript version)
...
function generateSyntheticHotel() {
  ...
  return {
    ...
    surface_totale: faker.number.float({ min: 500, max: 5000, fractionDigits: 2 }),
...
function generateSyntheticRoom(floorCount = 5) {
  ...
  return {
    ...
    statut: faker.helpers.weightedArrayElement([
      { value: 'disponible', weight: 60 },
      { value: 'occupee', weight: 35 },
...
function generateSyntheticEquipmentAssociation() {
  ...
  return {
    ...
    prix_supplement: !isGratuit ? faker.number.float({ min: 5, max: 50, fractionDigits: 2 }) : undefined
  };
}
...
function generateUpdateData(entityType) {
  switch (entityType) {
    ...
    case 'room':
      return {
        prix: faker.number.float({ min: 40, max: 300, fractionDigits: 2 }),
    ...
    case 'equipment':
      return {
        ...
        prix_supplement: faker.number.float({ min: 5, max: 100, fractionDigits: 2 }),
